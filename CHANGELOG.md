...
- Conta do agente no Teams (Configurações > Meu agente): o assistente entra logado com uma conta Microsoft dedicada, em vez de convidado sem conta. Sessão gerada por `npm run teams:login`, guardada cifrada, sem senha; o nome da conta precisa dizer que é a ata (constituição 2.2.0).
...
- Agent account on Teams (Configurações > Meu agente): the assistant joins signed in with a dedicated Microsoft account instead of as a guest with no account. Session generated by `npm run teams:login`, stored encrypted, no password; the account name must say it is the minutes (constitution 2.2.0).
...
- Cuenta del agente en Teams (Configurações > Meu agente): el asistente entra con sesión iniciada con una cuenta Microsoft dedicada, en vez de invitado sin cuenta. Sesión generada con `npm run teams:login`, guardada cifrada, sin contraseña; el nombre de la cuenta debe decir que es el acta (constitución 2.2.0).
